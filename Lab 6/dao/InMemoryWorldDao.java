...
//        }
        ...
        Map<String, City> highestPopulatedCitiesByCountry = allCities.values().stream()
                .collect(Collectors.toMap(City::getCountryCode, Function.identity(), BinaryOperator.maxBy(Comparator.comparingInt(City::getPopulation))));

//        for (Map.Entry<String, City> entry : highestPopulatedCitiesByCountry.entrySet()) {
//            String countryCode = entry.getKey();
//            City city = entry.getValue();
//
//            System.out.println("Country Code: " + countryCode + ", Highest Populated City: " + city.getName() + ", Population: " + city.getPopulation());
//        }

        Map<String, City> highestPopulatedCitiesByContinent = allCountries.values().stream()
                .flatMap(country -> country.getCities().stream())
                .collect(Collectors.toMap(city -> allCountries.get(city.getCountryCode()).getContinent(), Function.identity(), BinaryOperator.maxBy(Comparator.comparingInt(City::getPopulation))));

        highestPopulatedCitiesByContinent.forEach((continent, city) -> {
        ...
        Optional<City> highestPopulatedCapital = allCountries.values().stream()
                .flatMap(country -> country.getCities().stream()
